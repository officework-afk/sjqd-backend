...
function parseInvoice(text, pageType) {
    ...
    if (itemLine) {
        ...
        const rateMatch = itemLine.match(/(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(Nos|PCS)?\s+(\d{1,3}(?:,\d{3})*(?:\.\d+)?)/i);
    ...
    const totalAmount = n(findFirst(oneLine, [
