...
export const getPurchaseTotal = async (req, res) => {
    try {
        const userId = getUserId(req);
        const purchases = await prisma.purchase.findMany({
            where: { userId },
        });
        const total = purchases.reduce((sum, item) => sum + Number(item.totalAmount || 0), 0);
        res.json({ total });
    }
    catch (error) {
...
export const createPurchase = async (req, res) => {
    try {
        const userId = getUserId(req);
        const { invoiceNo, supplierName, itemName, quantity, rate, gstNo, gstRate, taxableAmount, cgst, sgst, igst, totalAmount, } = req.body;
...
export const importPurchases = async (_req, res) => {
    res.status(400).json({ message: "Excel import will be handled in next phase" });
};
export const extractPurchaseBill = async (_req, res) => {
    res.status(400).json({
