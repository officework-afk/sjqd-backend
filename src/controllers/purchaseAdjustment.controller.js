...
const buildStockChangeMap = (originalLines, revisedLines) => {
    ...
    return Array.from(keys)
        ...
        const stockChange = round2(toNumber(revised?.quantity) - toNumber(original?.quantity));
...
const resolveBodyTotals = (body, linesSummary, fallbackTotal) => {
    const adjustedTotalAmount = round2(toNumber(body?.adjustedTotalAmount ||
        body?.grandTotal ||
        body?.revisedTotalAmount) || linesSummary.totalAmount);
...
export const getPurchaseAdjustments = async (req, res) => {
    try {
...
export const createPurchaseAdjustment = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
        }
        const { noteNo, originalInvoiceNo, supplierName, gstNo, dueDate, phone, state, pincode, address, items, originalItems, itemName, quantity, rate, gstRate, discountType, discountValue, } = req.body;
...
export const updatePurchaseAdjustment = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
...
export const deletePurchaseAdjustment = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
