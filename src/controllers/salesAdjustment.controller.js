...
const normalizeLine = (raw, gstNo) => {
    const quantity = toNumber(raw?.quantity);
    const rate = toNumber(raw?.rate);
    const gstRate = toNumber(raw?.gstRate);
    const discountType = raw?.discountType === "amount" ? "amount" : "percentage";
    const discountValue = toNumber(raw?.discountValue);
    const subtotalAmount = toNumber(raw?.subtotalAmount) || round2(quantity * rate);
    const rawDiscount = discountType === "percentage"
        ? (subtotalAmount * discountValue) / 100
        : discountValue;
    const discountAmount = round2(Math.min(Math.max(rawDiscount, 0), subtotalAmount));
    const taxableAmount = round2(toNumber(raw?.taxableAmount) || subtotalAmount - discountAmount);
    ...
    const totalAmount = round2(toNumber(raw?.totalAmount) || taxableAmount + cgst + sgst + igst);
    return {
...
const coerceLines = (rawItems, fallback, gstNo) => {
...
const summarizeLines = (lines) => {
    ...
    return {
        ...
        quantity: round2(lines.reduce((sum, line) => sum + toNumber(line.quantity), 0)),
        rate: toNumber(first?.rate),
        gstRate: lines.length > 1 ? 0 : toNumber(first?.gstRate),
        subtotalAmount: round2(lines.reduce((sum, line) => sum + toNumber(line.subtotalAmount), 0)),
        discountAmount: round2(lines.reduce((sum, line) => sum + toNumber(line.discountAmount), 0)),
        taxableAmount: round2(lines.reduce((sum, line) => sum + toNumber(line.taxableAmount), 0)),
        ...
        totalAmount: round2(lines.reduce((sum, line) => sum + toNumber(line.totalAmount), 0)),
    };
};
const detectNoteType = (originalTotalAmount, adjustedTotalAmount) => {
    if (adjustedTotalAmount < originalTotalAmount)
        return "CREDIT";
    if (adjustedTotalAmount > originalTotalAmount)
        return "DEBIT";
    return null;
...
const generateAdjustmentNoteNo = async (db, userId, noteType, excludeId) => {
    ...
    const count = await db.salesAdjustment.count({ where });
    return `${prefix}-${year}-${String(count + 1).padStart(4, "0")}`;
};
const buildStockChangeMap = (originalLines, revisedLines) => {
    ...
    return Array.from(keys)
        ...
        const stockChange = round2(toNumber(original?.quantity) - toNumber(revised?.quantity));
...
const applyStockDifference = async (db, userId, originalLines, revisedLines) => {
    ...
    for (const change of changes) {
        ...
        if (item) {
            await db.item.update({
                where: { id: item.id },
                data: {
                    currentStock: round2(toNumber(item.currentStock) + toNumber(change.stockChange)),
                },
            });
            continue;
        }
...
const resolveBodyTotals = (body, linesSummary, fallbackTotal) => {
    const adjustedTotalAmount = round2(toNumber(body?.adjustedTotalAmount || body?.grandTotal || body?.revisedTotalAmount) ||
        linesSummary.totalAmount);
    const originalTotalAmount = round2(toNumber(body?.originalTotalAmount) || fallbackTotal);
...
export const getSalesAdjustments = async (req, res) => {
    try {
        ...
        console.error("GET SALES ADJUSTMENT ERROR:", error);
        res.status(500).json({ message: "Failed to fetch sales adjustment notes" });
    }
};
export const createSalesAdjustment = async (req, res) => {
    try {
        ...
        const { noteNo, originalInvoiceNo, partyName, gstNo, dueDate, phone, state, pincode, address, items, originalItems, itemName, quantity, rate, gstRate, discountType, discountValue, } = req.body;
        const revisedLines = coerceLines(items, {
            itemName,
            quantity,
            rate,
            gstRate,
        ...
        if (!String(partyName || "").trim()) {
            return res.status(400).json({ message: "Customer name is required" });
        }
        if (revisedLines.length === 0) {
            return res.status(400).json({ message: "At least one revised item is required" });
        }
        if (sourceLines.length === 0) {
            return res.status(400).json({
                message: "Original invoice items are required to create an adjustment note",
            });
        }
        const revisedSummary = summarizeLines(revisedLines);
        const sourceSummary = summarizeLines(sourceLines);
        const { originalTotalAmount, adjustedTotalAmount } = resolveBodyTotals(req.body, revisedSummary, sourceSummary.totalAmount);
        ...
        const created = await prisma.$transaction(async (tx) => {
            const finalNoteNo = String(noteNo || "").trim() ||
        ...
        console.error("CREATE SALES ADJUSTMENT ERROR:", error);
        res.status(500).json({ message: "Failed to create sales adjustment note" });
    }
};
export const updateSalesAdjustment = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        if (!existing) {
            return res.status(404).json({ message: "Sales adjustment note not found" });
        }
        const { noteNo, originalInvoiceNo, partyName, gstNo, dueDate, phone, state, pincode, address, items, originalItems, itemName, quantity, rate, gstRate, discountType, discountValue, } = req.body;
        const revisedLines = coerceLines(items, {
            itemName,
        ...
        const sourceLines = coerceLines(originalItems, null, gstNo || existing.gstNo || "B2C");
        const previousRevisedLines = coerceLines(existing.items, null, existing.gstNo || "B2C");
        const previousSourceLines = coerceLines(existing.originalItems, null, existing.gstNo || "B2C");
        if (!String(partyName || "").trim()) {
            return res.status(400).json({ message: "Customer name is required" });
        }
        if (revisedLines.length === 0) {
        ...
        const updated = await prisma.$transaction(async (tx) => {
            const finalNoteNo = existing.noteType === noteType
            ...
            return tx.salesAdjustment.update({
                ...
                data: {
                    noteNo: finalNoteNo,
                    noteType,
                    originalInvoiceNo: String(originalInvoiceNo || existing.originalInvoiceNo || "").trim() || "-",
                    partyName: String(partyName || "").trim(),
                    itemName: revisedSummary.itemName || itemName || "-",
                    quantity: revisedSummary.quantity,
                    rate: revisedSummary.rate,
        ...
        console.error("UPDATE SALES ADJUSTMENT ERROR:", error);
        res.status(500).json({ message: "Failed to update sales adjustment note" });
    }
};
export const deleteSalesAdjustment = async (req, res) => {
    try {
        const userId = getUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const sourceLines = coerceLines(existing.originalItems, null, existing.gstNo || "B2C");
        ...
        res.json({ message: "Sales adjustment note deleted successfully" });
    }
    catch (error) {
        console.error("DELETE SALES ADJUSTMENT ERROR:", error);
        res.status(500).json({ message: "Failed to delete sales adjustment note" });
    }
};
