...
const normalizeItemKey = (value: string) => String(value || "").trim().toLowerCase();
...
const detectNoteType = (originalTotalAmount: number, adjustedTotalAmount: number) => {
  ...
  return Array.from(keys)
    .map((key) => {
      ...
      const stockChange = round2(
        toNumber(original?.quantity) - toNumber(revised?.quantity),
      );
...
const resolveBodyTotals = (
  ...
  const adjustedTotalAmount = round2(
    toNumber(body?.adjustedTotalAmount || body?.grandTotal || body?.revisedTotalAmount) ||
