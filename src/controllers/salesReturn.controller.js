...
function generateReturnNo(prefix, id) {
    const year = new Date().getFullYear();
    const number = String(id).padStart(4, "0");
    return `${prefix}-${year}-${number}`;
}
function calculateGst(quantity, rate, gstRate) {
...
export const createSalesReturn = async (req, res) => {
    try {
        const userId = getUserId(req);
        const { returnNo, originalInvoiceNo, partyName, itemName, quantity, rate, gstNo, gstRate, } = req.body;
...
export const updateSalesReturn = async (req, res) => {
    try {
        ...
        if (oldItem) {
            const reversedStock = Number(oldItem.currentStock || 0) - Number(oldReturn.quantity || 0);
            if (reversedStock < 0) {
                return res.status(400).json({
